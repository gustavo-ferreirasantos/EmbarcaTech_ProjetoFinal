...
void ssd1306_draw_border(ssd1306_t *ssd, uint8_t y, uint8_t x1, uint8_t x2){
    int index_font = 69;
    
    for(uint8_t index_x = x1; index_x < x2; index_x++){
        // Desenha linha superior
        for (uint8_t i = 0; i < 8; ++i){
            uint8_t line = font[67 * 8 + i];
            for (uint8_t j = 0; j < 8; ++j){
                ssd1306_pixel(ssd, index_x * 8 + i, y + j, line & (1 << j));
            }
        }
        ...
        for (uint8_t i = 0; i < 8; ++i){
            uint8_t line = font[67 * 8 + i];
            for (uint8_t j = 0; j < 8; ++j){
                ssd1306_pixel(ssd, index_x * 8 + i, y+15 + j, line & (1 << j));
            }
        }
    }
    
    
    for(uint8_t index_y = 0; index_y < 2; ++index_y, index_font++){
        // Desenha linha da esquerda
        for (uint8_t i = 0; i < 8; i++){
            uint8_t line = font[index_font * 8 + i];
            for (uint8_t j = 0; j < 8; ++j){
                ssd1306_pixel(ssd, x1*8 + i, index_y * 8 + y + j, line & (1 << j));
            }
        }
    }
    ...
    for(uint8_t index_y = 0; index_y < 2; ++index_y, ++index_font){
        ...
        for (uint8_t i = 0; i < 8; i++){
            uint8_t line = font[index_font * 8 + i];
            for (uint8_t j = 0; j < 8; ++j){
                ssd1306_pixel(ssd, x2*7.5 + i, index_y * 8 + y + j, line & (1 << j));
            }
        }
    }
